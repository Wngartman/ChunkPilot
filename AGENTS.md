...
- Modrinth, optional CurseForge access through ChunkPilot's separately deployed application service (the application key stays server-side), GitHub Releases, Mojang, Paper, Purpur, official loader metadata, direct manifests, local packages, and future provider adapters. Personal CurseForge keys remain a native protected fallback for builds without the service. Never bundle or redistribute a shared developer key, pass credentials through React, or bypass provider distribution restrictions. A configured service is not proof that its deployment is online or accepted; verify it before enabling it in a release.
