...
try {
    ...
    foreach ($placeholder in '{{RELEASE_TAG}}', '{{INSTALLER_NAME}}', '{{HOTFIX_NOTES}}') {
    ...
    Write-Host "Watching exact release run: $runUrl"
    & gh run watch $runId --repo $Repository --exit-status
    if ($LASTEXITCODE -ne 0) { throw "Release workflow failed: $runUrl" }
    ...
    $metadata = Join-Path $verificationRoot "ChunkPilot-Release-Metadata-$tag.zip"
    $metadataRoot = Join-Path $verificationRoot 'metadata'
    Expand-Archive -LiteralPath $metadata -DestinationPath $metadataRoot
    $manifest = Get-Content -LiteralPath (Join-Path $metadataRoot 'build-manifest.json') -Raw | ConvertFrom-Json
    if ($manifest.ReleaseTag -ne $tag -or $manifest.GitSha -ne $head) {
        throw 'Public release metadata is not bound to the exact release request.'
    }
