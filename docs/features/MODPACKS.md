...
official Modrinth `.mrpack` releases, deterministic CurseForge server-pack contracts behind the
application credential gate, and local `.mrpack` files.
...
- Search, explicit submit, compatible Minecraft/loader/category filters and project details use the
  selected provider's official API. Modrinth uses v2. CurseForge remains unavailable in public builds
  until the approved application credential model is provisioned; users are never asked for a key.
...
- CurseForge production access is not active. Its typed contracts and deterministic fixtures are ready,
  but the desktop cannot safely embed an application secret and no approved gateway is configured.
