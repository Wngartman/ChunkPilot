...
| CurseForge | Project links, files, and server-pack contracts | Public access is disabled while ChunkPilot's application credential model is approved. No key is bundled and users are never asked for one. Deterministic fixtures cover the production contract without scraping. |
...
CurseForge provider contracts are developer-gated. A local developer may enable the provider only
through an explicit environment gate and the native secret store; neither is exposed through the
WebUI or bridge. The public build shows `CurseForge integration is being activated for ChunkPilot.`
until an approved application credential deployment exists. See [MODPACKS.md](MODPACKS.md).
