...
ChunkPilot's default archives live outside server roots under the configured data root's `Backups\<sanitized server name>` folder. A destination inside or equal to the source tree is rejected to prevent recursive backup growth. An ordinary ancestor folder is permitted and does not exclude the server's files. Source and destination paths must not pass through a symbolic link or junction.

Backup creation streams source files into a temporary ZIP, computes SHA-256 hashes, writes a manifest, closes the archive, verifies it **while it is still `.partial`**, and only then promotes it to the final name. A backup that fails verification is never renamed into place and no record is written for it. A profile that explicitly disables verification produces an unverified record; restore still verifies the complete archive before writing anything. Failed temporary archives and their orphaned manifests are cleaned, and a cleanup failure never replaces the exception that explains the real problem.
...
Retention is applied only to records and archives owned by the selected server/profile. Count, age, and storage limits remove the oldest eligible ChunkPilot backup; imported server content is never a retention target. The newest archive and the newest existing verified archive are protected even when they exceed the configured budget. Default profiles have a stable per-server identity so future manual backups share retention. Historical random profile identities remain untouched.

Restore requires a stopped server and explicit confirmation. The Agent first creates a verified pre-restore safety backup without running retention, preserving the selected archive even when it is old or over budget. Both backup and server identities must match the embedded manifest. The manifest must describe every archive member exactly once, with matching lengths and SHA-256 hashes. Duplicate, unlisted, case-aliased, conflicting, and unsafe Windows paths are rejected. Existing valid archives keep their original format and remain restorable.

Verification and extraction share one archive handle that prevents external replacement or writes. All destinations, including their existing ancestors and final files, are checked before mutation and rechecked while holding the shared path lock. Incoming files and readable originals are staged before activation; replacements are atomic per file. This remains an overlay restore: files absent from the archive are preserved.

An ordinary failure or cancellation rolls back activated files. A destination changed or locked by another program is preserved, and original recovery files remain in the staging directory named by the error. A process termination during activation does not automatically replay this rollback; the verified pre-restore ZIP remains available for manual recovery. This path does not claim a filesystem-wide transaction against a hostile concurrent process changing paths between Windows operations.

Synthetic integration coverage includes retention, historical archives, malformed inventories, cancellation, external edits, locks, and junction replacement. File-symlink fixtures run when the Windows test process already has that capability; otherwise the runner reports an explicit `UNAVAILABLE` skip. The tests never enable Developer Mode or acquire administrator privileges.
