...
ChunkPilot WebUI is a presentation replacement behind `--webui-preview`, not a second backend. The accepted WPF shell remains the default. The preview uses one native WPF window, one persistent Microsoft WebView2 control, and one locally bundled React application.
