...
| `api.curseforge.com` | Developer-gated CurseForge discovery, version/category inventory, link resolution, exact project/file detail, and update checks | No; only while the user requests a CurseForge browse, link, update, or refresh workflow | Search text, selected Minecraft version/loader/category, project/file IDs, public IP inherent in HTTPS, and the approved application credential in the `x-api-key` header | Five-second connect and 20-second total request bounds; redirects/cookies disabled; JSON capped at 8 MiB; identical in-flight requests coalesced. Completed API responses are not persisted under the currently reviewed third-party terms, so no offline CurseForge cache is claimed |
| `forgecdn.net` and its HTTPS subdomains, including `media.forgecdn.net` and `mediafilez.forgecdn.net` | User-selected CurseForge image, exact mod/modpack download, generated-candidate build, or update | No; only for visible provider images or an explicit download/install/update workflow | Exact provider-selected file/image path, public IP inherent in HTTPS, and for protected downloads the approved application credential in the `x-api-key` header | Redirects/cookies disabled. Archives are streamed with declared size and provider SHA-1 verification plus a local SHA-256 baseline. Images are natively fetched, type/size/dimension checked, converted to PNG, and held in a bounded memory cache |
...
managed server directory, world, configuration, log, or backup to CurseForge. Public credential delivery is
still gated: the development candidate can import the approved local key file into DPAPI-protected native
storage, but no key is committed, packaged, placed in React state, or accepted through an ordinary user field.
