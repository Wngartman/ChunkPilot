...
function connectivity(selected: ServerSummary | null): ConnectivitySnapshot | null {
  ...
  return {
    ...
    addresses: { local: selected.localAddress, lan: selected.lanAddress, publicVerified: null, routerReported: null, publicVerifiedAt: null },
    router: {
...
export class FixtureBridge implements BridgeAdapter {
  ...
  constructor(name = 'several') {
    ...
    if (this.current.connectivity && mode === 'connectivity-public') {
      this.current.connectivity.mode = 'PortForwarding'; this.current.connectivity.modeTitle = 'Internet hosting';
      ...
      this.current.servers = this.current.servers.map(item => item.id === this.current.selectedServerId ? { ...item, publicAddress: '203.0.113.24:25565', publicReachability: 'confirmed' } : item);
    }
