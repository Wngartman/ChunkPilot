...
export function CreateServerPage({ onDone, onOpenProviderSettings }: { onDone: () => void; onOpenProviderSettings?: () => void }) {
  ...
  const hasExactSelection = platform === 'Modpack' ? modpack !== null && (modpack.kind === 'local' || modpack.release.canCreate) : (selectedVersion?.selectable === true || (platform === 'Vanilla' && legacyArtifact?.minecraftVersion === selectedVersion?.id)) && (platform === 'Vanilla' ||
