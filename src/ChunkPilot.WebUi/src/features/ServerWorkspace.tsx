...
import { missingInstalledBuildTitle, versionHealthLabel } from './versions/presentation';
...
export function ServerWorkspace({ serverId, onOpenHelp = () => undefined }: { serverId: string; onOpenHelp?: (articleId: string) => void }) {
  ...
  return <div className={styles.workspace} data-console={tab === 'console' || undefined}>
  ...
  return <>
    ...
    <div className={styles.overviewWorkbench}>
      ...
      <aside className={styles.statusRail}>
        <div className={styles.statusItem}><header><Wifi size={15} /><span>Connection</span></header><strong>{server.connection?.serverId === server.id ? ({ computer: 'This computer', home: 'Home network', internet: 'Internet' } as const)[server.connection.audience] : 'Not available'}</strong><p>See How to join for current access.</p></div>
        <div className={styles.statusItem}><header><Users size={15} /><span>Players</span></header><strong>{server.playersOnline == null ? 'Unknown' : `${server.playersOnline}${server.playersMaximum == null ? '' : ` of ${server.playersMaximum}`} online`}</strong><p>{server.playersOnline == null ? (server.state === 'Stopped' ? 'Live count unavailable while stopped.' : 'No current count reported.') : 'Reported by the server'}</p></div>
...
function FilesPage({ server }: { server: ServerSummary }) {
  ...
  const navigate = useGuardedNavigation();
  const discard = useCallback(() => setDraft(loaded?.content ?? ''), [loaded]);
  useUnsavedChangesGuard(loaded !== null && draft !== loaded.content, discard, 'Your file changes have not been saved.');
  useEffect(() => {
  ...
  const open = (file: (typeof files)[number]) => navigate(() => {
...
function ModpackPage({ server }: { server: ServerSummary }) {
  ...
  if (!pack || !identity || !update?.sourceLinked || !updatePresentation)
    return <section className={styles.panel}><EmptyState title="Pack identity unavailable" detail="ChunkPilot has not established an exact provider project and release for this server. Pack-level updates are disabled until identity is proven." /></section>;
  return <div className={styles.modpackPage}>
    ...
    <section className={styles.packEvidence}>
      ...
      <footer className={styles.packDestinations}><span><strong>Versions</strong> contains verified recovery snapshots and rollback history.</span><span>Installed mods are listed below. Updates remain whole-pack only.</span></footer>
    </section>
    <details className={styles.packInventory} onToggle={event => setShowInventory(event.currentTarget.open)}>
      <summary>Installed mods · {snapshot.plugins.length}</summary>
      <p>Read-only JAR inventory. Individual mod updates are disabled for this pack. Per-file pack ownership is not available in this view.</p>
      {showInventory && (snapshot.plugins.length ? <InstalledAddonInventory items={snapshot.plugins} kind="mods" /> : <p>No mod inventory has been reported.</p>)}
    </details>
  </div>;
}

function InstalledAddonInventory({ items, kind, renderActions }: {
  items: PluginInventoryEntry[]; kind: 'plugins' | 'mods'; renderActions?: (plugin: PluginInventoryEntry) => ReactNode;
}) {
  return <table data-readonly={!renderActions || undefined} className={`${styles.table} ${pluginStyles.pluginResponsiveTable} ${pluginStyles.pluginInstalledTable}`}><thead><tr><th>{kind === 'mods' ? 'Mod' : 'Plugin'}</th><th>Version</th><th>JAR state</th><th>Load health</th><th>Compatibility</th>{renderActions && <th />}</tr></thead><tbody>{items.map(plugin => <tr key={plugin.relativePath}><td><strong>{plugin.name}</strong><small className={styles.cellMeta}>{plugin.fileName} · {bytes(plugin.sizeBytes)}</small></td><td>{plugin.version}</td><td><StatusBadge tone={plugin.enabled ? 'success' : 'neutral'}>{plugin.enabled ? 'Active' : 'Disabled'}</StatusBadge></td><td><StatusBadge tone={plugin.loadState === 'Failed' ? 'danger' : plugin.loadState === 'Loaded' ? 'success' : plugin.loadState === 'Pending' ? 'info' : 'neutral'} title={plugin.loadEvidence}>{plugin.loadState}</StatusBadge></td><td><StatusBadge tone={plugin.compatibility === 'Incompatible' ? 'danger' : plugin.compatibility === 'Unknown' ? 'warning' : 'success'}>{plugin.compatibility}</StatusBadge></td>{renderActions && <td><div className={styles.tableActions}>{renderActions(plugin)}</div></td>}</tr>)}</tbody></table>;
}
...
function AddonsPage({ server, kind }: { server: ServerSummary; kind: 'plugins' | 'mods' }) {
  ...
  const updateIdentityKey = useMemo(() => snapshot.plugins
    .filter(plugin => (plugin.provider === 'Modrinth' || plugin.provider === 'CurseForge') && plugin.providerProjectId)
    .slice(0, 100)
  ...
  const installRemote = () => {
    ...
    void command<ManagedContentOperation>(methods.install, {
      ...
      provider: selectedRelease.provider,
  ...
  const installDependency = (dependency: PluginRelease['dependencies'][number]) => {
    ...
    void command<ManagedContentOperation>(methods.install, { serverId: server.id, projectId: dependency.projectId, versionId: dependency.versionId, provider: release?.provider ?? provider, restartIfRunning: running, operationId: crypto.randomUUID() })
      .then(operation => {
        setContentOperations(current => [operation, ...current.filter(item => item.operationId !== operation.operationId)]);
        setContentPollEpoch(value => value + 1);
      });
  ...
  const installedFor = (projectId: string, versionId?: string) => snapshot.plugins.find(plugin =>
    plugin.provider === (release?.provider ?? provider) && plugin.providerProjectId === projectId &&
  ...
  return <div className={pluginStyles.pluginLayout}>
    <section className={styles.panel}>
      ...
      {section === 'installed' && (snapshot.plugins.length ? <InstalledAddonInventory items={snapshot.plugins} kind={kind} renderActions={plugin => <><Button variant="subtle" onClick={() => setConfigPlugin(plugin)}>Configure</Button><Button variant="subtle" disabled={!canApply || operationBusy} onClick={() => setPending({ kind: 'toggle', path: plugin.relativePath, name: plugin.name, enabled: !plugin.enabled })}>{plugin.enabled ? 'Disable' : 'Enable'}</Button><Button variant="subtle" disabled={!canApply || operationBusy} onClick={() => setPending({ kind: 'remove', path: plugin.relativePath, name: plugin.name })}>Remove</Button></>} /> : <EmptyState title={`No ${singular} JARs found`} detail={`Install a local JAR or browse compatible releases from an official provider. ChunkPilot inspects metadata without executing ${singular} code.`} />)}
    ...
    <ConfirmDialog open={pendingInstall !== null} title={`Apply ${pendingInstall?.versionName ?? `${singular} release`}?`} detail={pendingInstall ? `${pendingInstall.provider} · ${pendingInstall.fileName} · ${bytes(pendingInstall.sizeBytes)} · Minecraft ${pendingInstall.minecraftVersion} · ${pendingInstall.loader}. The official provider hash is verified before activation.${isMod ? ` ${clientRequirementLabel(pendingInstall.clientRequirement)}.` : ''}${running ? ' The running server will save, stop, apply the reversible change, and restart.' : ''}` : ''} confirmLabel={running ? 'Apply and restart' : `Apply ${singular} change`} onCancel={() => setPendingInstall(null)} onConfirm={installRemote} />
    ...
    <section className={styles.versionEvidence}>
      ...
      {isPaper
        ? installedPaperBuild
          ...
          : <EmptyState title={missingInstalledBuildTitle('Paper', server.loaderVersion)} detail={!server.loaderVersion?.trim() ? 'The exact installed version was not recorded, so catalog support cannot be matched.' : catalogError || paperBuilds?.message || 'Refresh the official PaperMC catalog to resolve this installed build.'} />
        : isLoader
          ? installedLoaderBuild
            ...
            : <EmptyState title={missingInstalledBuildTitle(loaderPlatform, server.loaderVersion)} detail={!server.loaderVersion?.trim() ? 'The exact installed version was not recorded, so catalog support cannot be matched.' : catalogError || loaderBuilds?.message || `Refresh the official ${loaderPlatform} catalog to resolve this installed build.`} />
          : installedEvidence
...
function ConnectivitySettings({ server }: { server: ServerSummary }) {
  ...
  const joining = connectionChoice(server);
  ...
  return <div className={styles.connectivityPage}>
    <header className={styles.connectivityHeader}><div><h2>Connectivity</h2><p>Choose how people join.</p></div><StatusBadge tone={joining.tone}>{joining.badge}</StatusBadge></header>
    ...
    <section className={styles.addressSection}><div className={styles.sectionHeading}><div><h3>How people connect</h3><p>Addresses reflect the current listener, not just your saved preference.</p></div></div><ConnectionSummary server={server} connectivity={connectivity} showAll /></section>
    {connectivity.mode === 'PortForwarding' && <>
...
function Toggle({ label, value, onChange }: { label: string; value: boolean; onChange: (value: boolean) => void }) { return <button type="button" className={page.toggle} data-checked={value} role="switch" aria-label={label} aria-checked={value} onClick={() => onChange(!value)} />; }
