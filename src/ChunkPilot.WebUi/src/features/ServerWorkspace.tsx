...
function PlayersPage({ server }: { server: ServerSummary }) {
  ...
  return <div className={styles.playerWorkspace}>
    <section className={styles.playerSummary}>
      ...
      <div><span>Allowlist</span><strong>{access ? access.whitelistEnabled ? 'On' : 'Off' : 'Unavailable'}</strong><small>{access?.supportsAllowlist ? canModerate ? 'Changes apply through the running server.' : 'Start the server to make changes.' : 'Support has not been confirmed for this server.'}</small></div>
    ...
    <section className={styles.panel}>
      ...
      {players.length ? <table className={styles.table}><thead><tr><th>Player</th><th>Status</th><th>Allowlist</th><th>Role</th><th aria-label="Actions" /></tr></thead><tbody>{players.map(player => <tr key={player.name}><td><strong>{player.name}</strong></td><td><StatusBadge tone={player.banned ? 'danger' : player.online ? 'success' : 'neutral'}>{player.banned ? 'Banned' : player.online ? 'Online' : 'Known player'}</StatusBadge></td><td>{player.allowlisted ? 'Allowed' : 'Not allowed'}</td><td>{player.operator ? 'Operator' : 'Player'}</td><td><div className={styles.tableActions}>{access?.supportsAllowlist && <Button variant="subtle" disabled={!canModerate} onClick={() => player.allowlisted ? confirm(player.name, 'RemoveFromWhitelist', 'Remove access', `${player.name} will no longer be able to join an allowlist-only server.`) : moderate(player.name, 'AddToWhitelist')}>{player.allowlisted ? 'Remove allowlist' : 'Allowlist'}</Button>}<details><summary aria-label={`Moderation actions for ${player.name}`}><MoreHorizontal size={16} /></summary><div className={styles.rowMenu}>{access?.supportsOperators && (player.operator ? <button disabled={!canModerate} onClick={() => confirm(player.name, 'RemoveOperator', 'Remove operator', `${player.name} will lose operator permissions.`)}>Remove operator</button> : <button disabled={!canModerate} onClick={() => moderate(player.name, 'GrantOperator')}>Make operator</button>)}{player.online && <button disabled={!canModerate} onClick={() => confirm(player.name, 'Kick', 'Kick player', `${player.name} will be disconnected from the running server.`)}>Kick</button>}{access?.supportsPlayerBans && (player.banned ? <button disabled={!canModerate} onClick={() => moderate(player.name, 'Pardon')}>Pardon</button> : <button disabled={!canModerate} className={styles.dangerAction} onClick={() => confirm(player.name, 'Ban', 'Ban player', `${player.name} will be banned and disconnected.`)}>Ban</button>)}</div></details></div></td></tr>)}</tbody></table> : <EmptyState title={emptyTitle} detail={emptyDetail} />}
