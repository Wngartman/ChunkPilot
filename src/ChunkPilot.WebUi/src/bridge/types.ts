...
export interface PluginInventoryEntry { name: string; fileName: string; relativePath: string; version: string; id: string; loader: string; sizeBytes: number; modifiedAt: string; enabled: boolean; duplicateId: boolean; dependencies: string[]; dependencyDetails: ContentDependency[]; compatibility: 'Compatible' | 'LikelyCompatible' | 'Incompatible' | 'Unknown'; compatibilityReason: string; loadState: 'Loaded' | 'Failed' | 'Pending' | 'Disabled' | 'Not running' | 'Unknown'; loadEvidence: string; installSource: string; provider?: 'Modrinth' | 'CurseForge' | 'Hangar' | null; providerProjectId?: string; providerVersionId?: string; sha256: string; clientRequirement?: 'ServerOnly' | 'ClientOptional' | 'ClientAndServer' | 'ClientOnly' | 'Unknown'; }
export interface PluginProviderStatus { provider: 'Modrinth' | 'CurseForge' | 'Hangar'; available: boolean; detail: string; }
