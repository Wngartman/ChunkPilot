...
public sealed partial class ManagedServer
{
        ...
        string name, string reason, SafeFileService files, CancellationToken cancellationToken)
    {
        try
        {
            return await new StoppedPlayerAccessService(files).ApplyAsync(Definition, action, name, reason,
                VerifyStoppedAccessAsync, cancellationToken).ConfigureAwait(false);
        }
    ...
    private async Task VerifyStoppedAccessAsync(CancellationToken cancellationToken)
    {
        if (State != ServerState.Stopped || HasDetachedProcess || HasExactOwnedProcessAlive() ||
            ...
            throw new InvalidOperationException("The server is not proven stopped. Resolve its process state before changing access files.");
        ...
        if (endpoints.GetActiveTcpListeners().Any(endpoint => endpoint.Port == Definition.Port) ||
            ...
            throw new InvalidOperationException("The configured server port is still in use. No access files were changed.");
    }
}
